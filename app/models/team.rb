class Team < ActiveRecord::Base
  ...
  OR home_team_id=? and games.visitor_team_ap_rank > 0  and games.complete=1 and games.home_score > games.visitor_score AND s.id=?', self.id, Game::SEASON,self.id,Game::SEASON],
               :include => {:week => :season})
  end
